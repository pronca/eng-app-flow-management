...
@Component({
  ...
  export class FlowErrorActivityComponent extends EditActivityComponent  {
    ...
    ngOnInit(): void {
        ...
        this.filterErrorCombo.extraDateFrom = starterParams.extraDateFrom;
        this.filterErrorCombo.extraDateTo = starterParams.extraDateTo;
        this.filterErrorCombo.canViewDateFromToFilters = starterParams.canViewDateFromToFilters;
    ...
    exportFileXLSX(name: String) {
        ...
        this.flowConfigurationService.downloadFlowErrorXlsx2(this.filterError).then(result => {
            if (result.size > 0) {
                ...
                } else if (this.filterError.name == 'ERRORI') {
                  anchor.download = "Errori Pratiche flusso " + this.fmflow.name;
                } else {
                  anchor.download = this.filterError.name + "_" + this.fmflow.name;
                }
                anchor.href = fileURL;
                anchor.click();
                // window.open(fileURL);
...
/**
  ...
  isEditModeEdit(){
    ...
    onCodiceAziendaChange(event: any): void {
      ...
      // 2?? Svuota codice presidio
      this.codicePresidio = "";
      this.filterError.codicePresidio = null;

      // 3?? Ricarica la lista dei presidi filtrata per azienda
      this.loadCodiciPresidio();

      // 4?? Avvia la ricerca aggiornata
      this.startSearch();
    }
