...
import { EditActivityComponent, ActivityManager, Activity, Promises }  from 'eng-app';
import { FlowConfigurationService } from '../../flow-configuration.service';



@Component({
    selector: 'flow-view-upload',
    templateUrl: './flow-view-upload.component.html',
    styleUrls: ['./flow-search-activity.components.scss']
  })
  export class FlowViewUploadComponent extends EditActivityComponent {
    fileName = '';
    data:any[];
    ...
    public fileChanged(event): void {
      ...
      this.readFile(inputFile).then((file) => {
          this.flowConfigurationService.importFlowFromTable(file).then(response => {
              ...
              if (response.success) {
                  ...
                  this.activityManager.engApplication.notifyMessage('Flusso già esistente');
              }
          }).catch((e) => {
