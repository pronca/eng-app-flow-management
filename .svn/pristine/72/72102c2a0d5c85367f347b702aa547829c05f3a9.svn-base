...
@Component({
  ...
  export class FlowErrorActivityComponent extends EditActivityComponent  {
    ...
    /**
     * ðŸ”¹ Carica i codici azienda da API
     */
...
/**
        ...
        this.flowConfigurationService.flowErrors(this.filterError).then(result => {
            if (result.success) {
              ...
              let errorMsg = '';
                ...
                // ðŸ”¹ Controllo multilivello su array errors â†’ text â†’ content
                if (result.errors) {
                ...
                //Se non Ã¨ stato trovato alcun messaggio valido
    ...
    generaColonnePratiche(colonneMap: any, nameMap: any): string[] {
      ...
      //se Ã¨ stata impostata la query dinamica non includo il campo numero errori
      if (!this.queryDetail) {
  ...
  isEditModeEdit(){
    ...
    /**
     * ðŸ”¹ Quando cambia il Codice Azienda:
    ...
    onCodiceAziendaChange(event: any): void {
      ...
      // 2ï¸âƒ£ Svuota codice presidio
      this.codicePresidio = "";
      this.filterError.codicePresidio = null;

      // 3ï¸âƒ£ Ricarica la lista dei presidi filtrata per azienda
      this.loadCodiciPresidio();

      // 4ï¸âƒ£ Avvia la ricerca aggiornata
      this.startSearch();
    }
