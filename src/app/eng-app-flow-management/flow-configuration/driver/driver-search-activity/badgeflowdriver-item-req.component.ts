...
@Component({
  selector: 'badgeflowdriver-item-req',
  templateUrl: './badgeflowdriver-item-req.component.html',
  styleUrls: ['./badgeflowdriver-item-req.component.scss']
})
export class BadgeFlowDriverViewItemRequest extends BadgeItemComponent {
  ...
  ngOnInit() {
    ...
    this.activity.addActivityAction({
      ...
      icon: "add",
      fn: (activity, action) => {
        this.activityManager.startChildActivityByName("driver.edit");
  ...
  openChildActivity(act2Name: string, asPopup: boolean) {
    ...
    this.activityManager.startChildActivityByName("driver.edit", startingParams);
  }
